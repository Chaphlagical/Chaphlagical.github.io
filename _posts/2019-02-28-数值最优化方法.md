...
$lim_{k \rightarrow \infin} \frac{\parallel x_{k+1}-x^ \star \parallel}{\parallel x_k-x^ \star \parallel}=a​$ 

#### 		2、超线性收敛

$lim_{k \rightarrow \infin} \frac{\parallel x_{k+1}-x^ \star \parallel}{\parallel x_k-x^ \star \parallel}=0$

#### 		3、二阶收敛

$lim_{k \rightarrow \infin} \frac{\parallel x_{k+1}-x^ \star \parallel}{\parallel x_k-x^ \star \parallel^2}=a​$ 
...
设在水平集$\{x \mid f(x) \leq f(x_0)\}$上，$f(x)$有下界，$g(x)$一致收敛，则非精确线搜索算法中的方向$d_k$与$-g_k$之间的夹角$\theta_k$一致有界，即对某一$\mu>0$，有 $0 \leq\theta_k \leq \frac{\pi}{2}-\mu，\forall k$.若Wolfe准则对任给k都成立，则或者存在N，使$g_N=0$，或者$g_k \rightarrow 0，k \rightarrow \infin.​$
...
**步1** 给定$\alpha_0 \in [0,\infin)，\gamma_0>0，t>1，i:=0$.
...
**步4** 若$i=0$，令$\gamma _i:=-\gamma_i，\alpha:=\alpha_{i+1}​$，转**步2**；否则

$a=min\{\alpha ，\alpha_{i+1}\}$

$b=miax\{\alpha ，\alpha_{i+1}\}​$
