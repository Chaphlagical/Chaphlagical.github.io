...
函数的内积：
$$
f\cdot g=\int^\infity_{-\infity}f(x)\cdot g(x)dx
$$
向量正交投影变换：
$$
\begin{align}
...
\begin{align}
a_k&=f\cdot e_k\\
&=\int^\infity_{-\infity}f(t)e_k(t)dt
\end{align}
$$
向量正交投影逆变换：
$$
...
**帕斯瓦尔定理**
$$
\int^\infity_{-\infity}|f(t)|^2dt=\int^\infity_{-\infity}|F(u)|^2du
$$
**卷积定理**
...
$$
g(x,y)=\iint^{+\infity}_{-\infity}f(\alpha,\beta)h(x-\alpha,y-\beta)d\alpha d\beta+n(x,y)
$$
以一维条件下为例（可以推广到二位情况），点$x$处的函数$f$为：
$$
f(x)=\int^\infity_{-\infity}f(x')\delta(x-x')dx'
$$
其中，$\delta(r)$表示增量函数
$$
g(x)=D(f(x))=D(\int^\infity_{-\infity}f(x')\delta(x-x')dx')
$$
若$D$为线性运算，可以变换$D$与积分符号次序
$$
g(x)=D(f(x))=\int^\infity_{-\infity}f(x')D(\delta(x-x'))dx'
$$
引入新的函数$h(x,x')=D(\delta(x-x'))$，假设$D$只与$x-x'$有关，则有
$$
g(x)=\int^\infity_{-\infity}f(x')h(x-x')dx'
$$
...
**卷积**
$$
f(t)*g(t)=\int^{\infity}_{-\infity}f(\tau)g(t-\tau)d\tau\leftrightarrow F(s)\ G(s)
$$
**相关**
$$
f(t)*g(-t)=\int^\infity_{-\infity}g(\tau)g(t+\tau)d\tau\leftrightarrow F(s)\ G^*(s)
$$
