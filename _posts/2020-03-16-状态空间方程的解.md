...
=\sum\limits^\infin_{k=0}\frac{1}{k!}(at)^k
\end{align}
$$
...
$$
e^{\boldsymbol S}=\sum\limits^\infin_{k=0}\frac{1}{k!}\boldsymbol S^k=\boldsymbol I+\boldsymbol S+\frac{1}{2!}\boldsymbol S^2+\frac{1}{3!}\boldsymbol S^3+\cdots
$$
对于线性定常系统，矩阵指数(状态转移矩阵)定义为：
$$
e^{\boldsymbol A t}=\sum\limits^\infin_{k=0}\boldsymbol A^kt^k=\boldsymbol I+\boldsymbol At+\frac{1}{2!}\boldsymbol A^2t^2+\frac{1}{3!}\boldsymbol A^3t^3+\cdots
$$
...
更进一步：
$$
e^{\boldsymbol A t}=\sum\limits^\infin_{k=0}\frac{1}{k!}\boldsymbol A^kt^k=\sum\limits^\infin_{k=0}\frac{t^k}{k!}\sum\limits^{n-1}_{m=0}\beta_m \boldsymbol A^m=\sum\limits^{n-1}_{m=0}\alpha_m(t)\boldsymbol A^m
$$
...
矩阵函数、标量函数的相互构造：
$$
f(\boldsymbol A)=e^{\boldsymbol At}=\sum\limits^\infin_{k=0}\frac{\boldsymbol A^kt^k}{k!}\Leftrightarrow f(\lambda)=\sum\limits^\infin_{k=0}\frac{\lambda^kt^k}{k!}=e^{\lambda t}
$$
构造另一个低阶多项式标量函数：
$$
